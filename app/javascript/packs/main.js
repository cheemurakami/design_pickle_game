...
shuffledImages.map((_,i) => {
  $(".card-wrapper").append(`<div class="card-container"><img class="card-image" data-position=${i} src=${hiddenImageUrl}></div>`)
...
$(".card-image").click((e) => {
  ...
  if(firstGuessedUrl === secondGuessedUrl){
    ...
    setTimeout(() => {
      $(`.card-image[data-position="${guessedPositionNum[0]}"]`).attr("src", hiddenImageUrl);
      $(`.card-image[data-position="${guessedPositionNum[1]}"]`).attr("src", hiddenImageUrl);
    }, 2000)
  }
})
